...
var _ = Describe("outcome test", func() {
	...
	Context("50 moves draw rule", func() {
		...
		// mm makes move
		mm := func(srcCoord, dstCoord string) { Expect(b.MakeMove(to(dstCoord), from(srcCoord))).To(BeTrue()) }

		// mms makes moves
		mms := func(coords [][2]string) {
			for i := range coords {
				By(fmt.Sprintf("making move %d...", b.MoveNumber()))
				mm(coords[i][0], coords[i][1])
			}
		}
		...
		It("plays full game, draw by 50 moves rule", func() {
			...
			mms([][2]string{
				...
				{`g6`, `e7`}, {`g8`, `h8`},
				{`e6`, `g5`}, /*{`a1`, `a6`}, // 113
				{`f6`, `f7`}, {`a6`, `f6`},*/ // todo investigate, should this be playable or not, count moves manually
			})
